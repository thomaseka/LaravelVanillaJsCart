...
<body>
    <div class="mx-auto max-w-screen-xl px-4 py-16 sm:px-6 lg:px-8  ">
        <div class="mx-auto max-w-lg mt-16">
            @if(session('error'))
            <div class="row">
                <div class="bg-red-600 text-white p-4 rounded-lg shadow-md grid place-items-center" role="alert">
                    {{session('error')}}
                </div>
            </div>
            @endif
            <form action="/login" method="POST" class="mb-0 space-y-4 rounded-lg p-4 shadow-lg sm:p-6 lg:p-8 bg-gray-200">
                @csrf
                <div class="">
                    <p class="text-center text-2xl font-bold text-blue-600 sm:text-3xl">Login </p>
                    <p class=" text-center text-gray-500">
                        Masukkan email dan password</p>
                </div>

                <!-- Nama -->
                <div>
                    <div class="relative">
                        <input type="text" name="email" id="email" value="{{ old('email') }}" class="block rounded-t-lg px-2.5 pb-2.5 pt-5 w-full text-sm text-gray-900 bg-gray-50 border-0 border-b-2 border-gray-300 appearance-none  focus:outline-none focus:ring-0 focus:border-blue-600 peer shadow-lg" placeholder="" value="{{ old('email') }}" required />
                ...
                <div>
                    <div class="relative">
                        <input type="password" name="password" id="password" value="{{ old('name') }}" class="block rounded-t-lg px-2.5 pb-2.5 pt-5 w-full text-sm text-gray-900 bg-gray-50  border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer shadow-lg" placeholder="" required />
                        ...
                        <span class="absolute inset-y-0 end-0 grid place-content-center px-4">
                            ...
                            <label for="togglePassword" id="passwordLabel" class="text-gray-500 cursor-pointer flex items-center justify-center">
                ...
                <div class="mt-4">
                    ...
                    <p class="text-center text-sm text-gray-500 mt-4">
                        Belum punya akun?
                        <a class="underline text-indigo-600" href="/register">Daftar</a>
                    </p>
                </div>
